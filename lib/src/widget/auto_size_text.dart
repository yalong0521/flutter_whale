...
class AutoSizeText extends StatefulWidget {
  ...
  @override
  State createState() => _AutoSizeTextState();
}
...
class _AutoSizeTextState extends State<AutoSizeText> {
  ...
  List _calculateFontSize(
    ...
    if (presetFontSizes == null) {
      final double defaultFontSize = style!.fontSize!.clamp(
        widget.minFontSize,
        widget.maxFontSize,
      );
      ...
      if (_checkTextFits(
        span,
        TextScaler.linear(defaultScale),
        maxLines,
        size,
      )) {
    ...
    while (left <= right) {
      ...
      double scale;
      if (presetFontSizes == null) {
        scale = userScaler.scale(
          mid * widget.stepGranularity / style!.fontSize!,
        );
      } else {
        scale = userScaler.scale(presetFontSizes[mid] / style!.fontSize!);
      }
      if (_checkTextFits(span, TextScaler.linear(scale), maxLines, size)) {
        left = mid + 1;
        lastValueFits = true;
      } else {
        right = mid - 1;
